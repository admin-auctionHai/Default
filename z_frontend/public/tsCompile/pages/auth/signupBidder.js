...
function SignUpPageTailwind() {
    ...
    const goToNextStepOfForm = (e) => __awaiter(this, void 0, void 0, function* () {
        e.preventDefault();
        ...
        if (isEmailVerified && isFormValid && companyFormValid) {
            ...
            try {
                const response = yield authService.bidderRegister(submissionData);
                if (response.token) {
                ...
                setErrors(prev => (Object.assign(Object.assign({}, prev), { submit: err.message })));
                setIsEmailVerified(false);
                setIsFormValid(false);
                setCompanyFormValid(false);
                setCurrentStep(1);
        ...
        if (true) {
            ...
            try {
                const response = yield authService.requestOtp();
    ...
    const handleRequestAgain = (e) => __awaiter(this, void 0, void 0, function* () {
        e.preventDefault();
        //Send the email to 
        try {
        }
    ...
    return (_jsx("div", { className: "flex justify-center items-center min-h-screen bg-gray-100", children: _jsxs("div", { className: "bg-white shadow-lg rounded-lg overflow-hidden max-w-6xl w-full", children: [currentStep === 1 &&
                    ...
                    _jsx("div", { id: "div-captcha-validation", className: "flex w-full pt-1 px-8", children: _jsx(CaptchaValidation, { ref: captchaRef }) }), currentStep === 1 &&
                    _jsx("div", { className: "w-full items-end", children: _jsx("button", { className: "py-2 px-6 my-4 border border-solid border-black hover:bg-gray-600 transition-colors duration-300", onClick: handleSubmit, children: "Submit" }) }), currentStep === 2 &&
                    _jsxs("div", { className: "w-full h-full flex flex-col items-center", children: [_jsx("h2", { className: "text-2xl p-2", children: "Verify Your Email" }), _jsx("div", { className: "p-4", children: _jsxs("div", { children: [_jsx("p", { children: "Kindly enter the OTP send on the Correspondence Email submit on Registration Form" }), _jsxs("div", { className: "flex-row", children: [_jsx(OTPVerification, { onVerify: handleOtpVerification, onRequestAgain: handleRequestAgain }), _jsx("div", { children: _jsx("button", { className: "m-3 p-2 right-0 border border-solid rounded-xl bg-gray-200 hover:bg-gray-500", onClick: handleBackToEditForm, children: "Edit Data. Go Back" }) })] })] }) })] })] }) }));
}
